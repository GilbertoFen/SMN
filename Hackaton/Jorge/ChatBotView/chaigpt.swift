...
struct ChatView: View
{
                ...
                ScrollViewReader { proxy in
                    ScrollView(.vertical, showsIndicators: false)
                    {
                        VStack(spacing: 16)
                        {
                            ...
                            {
                                SwipeCardsView(cards: travelCards)
                                    .padding(.top, 8)
            ...
            ToolbarItem(placement: .principal)
            {
                VStack(spacing: -32)
                {
                    ...
                    Text("Ayuda personalizada")
                        ...
                        .foregroundStyle(Color.black.opacity(0.45)).offset(x:0, y:30)
                }
            }
        }
    }
    ...
    func sendMessage()
    {
        ...
        guard let url = URL(string: "http://127.0.0.1:3000/chat") else
        {
            removeTypingBubble()
            ...
            do
            {
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.8)
                {
                    ...
                    else if let error = decoded.error
                    {
                        ...
                        items.append(Item(text: "Error: \(error)\n\(detailsText)", isFromUser: false))
                    }
                    else
                    {
